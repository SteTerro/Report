...
p1=ggplot(data = data.frame(sample=res), aes(sample = sample)) +
  stat_qq() +                   
  stat_qq_line(color = "red") +  
  labs(x = "Theorical quantiles", y = "Sample quantiles")

p2=ggplot(dati, aes(x = fitted(m5), y = res)) +
...
#test modello saturo
pchisq(m11$deviance, df = m11$df.residual, lower.tail = FALSE) #va messo?
...
ggplot(dati) +
  geom_point(aes(x = specie, y = fitted(f), color = "Normale"), size = 2, color="darkorchid3") +
  geom_point(aes(x = specie, y = fitted(m11), color = "Poisson"), size = 2, color="darkorange2") +
  # Linea diagonale di riferimento
  geom_abline(intercept = 0, slope = 1, color = "black") +
  # Etichette
  labs(x = "Species", y = "Estimated values", color = "Modello")

ggplot(dati) +
  geom_point(aes(x = specie, y = fitted(f), color = "Normale"), size = 2) +
  geom_point(aes(x = specie, y = fitted(m11), color = "Poisson"), size = 2) +
  # Linea diagonale di riferimento
  geom_abline(intercept = 0, slope = 1, color = "black") +
  # Etichette
  labs(x = "Species", y = "Estimated values", color = "Modello") + 
  scale_color_manual(values = c("Normale" = "darkorchid3", "Poisson" = "darkorange2"))+
...
```{r}
f<-lm(specie~area+ap+sr+hy)
summary(f)
add1(f,.~+(.)^2,test="F")
drop1(f,test="F")
AIC(f)
res11<-rstandard(f)
shapiro.test(res11)#normalit dubbia
ggplot(data = data.frame(sample=res11), aes(sample = sample)) +
  stat_qq() +                   
  stat_qq_line(color = "red") +  
  labs(x = "Theorical quantiles", y = "Sample quantiles")

ggplot(dati, aes(x = fitted(f), y = res11)) +
  geom_point(color = "blue", size = 2) +  
  labs(x = "Estimated values",y = "Model residuals") #omoschedasticità sembra ok
library(gridExtra)
grid.arrange(p1,p2,ncol=2)

ggplot(dati, aes(x = specie, y = fitted(f))) +
  geom_point(color = "blue", size = 2) +  
  labs(x = "Species",y = "Estimated values") +
  geom_abline(intercept = 0, slope = 1, color = "red")
cor(fitted(f),specie)
